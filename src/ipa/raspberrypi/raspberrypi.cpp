...
public:
	IPARPi()
		: haveLensCtrls_(false), controller_(), frameCount_(0), checkCount_(0),
		  ...
		  firstStart_(true), afMode_(controls::AfModeManual), afPauseFlag_(false),
		  afIdleFlag_(true)
	{
	}
...
void IPARPi::reportMetadata(unsigned int ipaContext)
{
	...
	if (afStatus) {
		...
		if (afPauseFlag_ && afMode_ == controls::AfModeContinuous) {
			p = afStatus->state == AfState::Scanning ?
				controls::AfPauseStatePausing : controls::AfPauseStatePaused;
		} else
...
void IPARPi::queueRequest(const ControlList &controls)
{
	...
	for (auto const &ctrl : controls) {
		...
		case controls::AF_METERING: {
			...
			if (!af) {
				LOG(IPARPI, Warning)
					<< "Could not set AF_METERING - no focus algorithm";
				break;
			}
		...
		case controls::AF_WINDOWS: {
			...
			if (!af) {
				LOG(IPARPI, Warning)
					<< "Could not set AF_WINDOWS - no focus algorithm";
				break;
			}
