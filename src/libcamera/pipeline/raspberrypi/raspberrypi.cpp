...
CameraConfiguration::Status RPiCameraConfiguration::validate()
{
	...
	for (StreamConfiguration &cfg : config_) {
		if (isRaw(cfg.pixelFormat)) {
			 ...
			 * the user request.
			 */
			V4L2DeviceFormat unicamFormat = findBestMode(data_->sensorFormats_, cfg.size);
							       ...
							       const StreamRoles &roles)
{
	...
	for (const StreamRole role : roles) {
		...
		case StreamRole::Raw:
			...
			unicamFormat = findBestMode(data->sensorFormats_, size);
			pixelFormat = unicamFormat.fourcc.toPixelFormat();
...
int PipelineHandlerRPi::start(Camera *camera, const ControlList *controls)
{
	...
	V4L2DeviceFormat ispFormat = toV4L2DeviceFormat(sensorFormat);
...
bool PipelineHandlerRPi::match(DeviceEnumerator *enumerator)
{
	 ...
	 * no transform at all.
	 */
	const V4L2Subdevice *sensorDev = data->sensor_->device();
	const struct v4l2_query_ext_ctrl *hflipCtrl = sensorDev->controlInfo(V4L2_CID_HFLIP);
	if (hflipCtrl) {
